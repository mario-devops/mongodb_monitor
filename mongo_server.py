...
class mongodbMonitor(object):
	...
	def get_mongo_monitor_data(self, conn):
		...
		if(mongo_role == 1):
			...
			mongodb_slow_query_role,slow_query_dict = self.slow_query(conn)
			repl_status_dict = {}
			...
			if (mongodb_slow_query_role == "master" ):
				mongo_monitor_dict.update(slow_query_dict)

				for i in dbs_items:
					db = i['dbname']
					conn[db].set_profiling_level(2,slow_ms=1000)
	...
	def slow_query(self,conn):
		...
		for d in db_name_list:

			serverStatus = conn.admin.command(pymongo.son_manipulator.SON([('serverStatus', 1)]))
			if (serverStatus.has_key("repl") and  serverStatus["repl"].has_key("secondary")):
				if serverStatus["repl"]["ismaster"]:
					mongodb_slow_query_role = "master"
				if  serverStatus["repl"]["secondary"]:
					mongodb_slow_query_role = "secondary"
			else: # not Replica sets mode
				mongodb_slow_query_role = "standalone"
			db_name_slow_log_number= d + "_slow_log_number"
			slow_query_dict[db_name_slow_log_number]=conn[d].system.profile.find({'millis': {'$gt': 5}}).count()
		print slow_query_dict
		return (mongodb_slow_query_role,slow_query_dict)
